...
// Start Quiz
function test() {
    let copy = ques.slice(); // clone array

    // shuffle copy by Fisher-Yates algorithm
    for (let i = copy.length - 1; i > 0; i--) {
        let j = Math.floor(Math.random() * (i + 1));
        [copy[i], copy[j]] = [copy[j], copy[i]]; // swap
    }

    random = copy.slice(0, 15); // pick first 15 from the shuffled array
    pos = 0;
    userchoice = [];
    showQuestions();
}
...
function escapeHTML(text) {
    return text.replace(/</g, "&lt;").replace(/>/g, "&gt;");//lt-- means literal tag starts(<) gt-- go tag ends(>)
}
...
function showQuestions() {
    ...
    text += "Q" + (pos + 1) + ". " + escapeHTML(q.question) + "<br><br>";

    for (let i = 0; i < q.options.length; i++) {
        // Keep previous selection checked
        text += "<input type='radio' name='opt"+pos+"' value='"+i+"' "+
        (userchoice[pos] === i ? "checked" : "") +
        "> " + escapeHTML(q.options[i]) + "<br><br>";
   ...
   text += "<div style='display:flex; justify-content:center; gap:20px; margin-top:20px;'>"
      + "<button onclick='previousQ()' class='btn btn-dark' style='background-color:#D7AA80; color:black; font-weight:bold;'>Previous</button>"
      + "<button onclick='nextQ()' class='btn btn-dark' style='background-color:#D7AA80; color:black; font-weight:bold;'>"
      + (pos === random.length - 1 ? "Finish" : "Next")
      + "</button>"
...
// Save current answer
function saveAns() {
    let choice = document.getElementsByName("opt" + pos);
...
let theme=document.querySelector("#myDropdown");
 theme.addEventListener('change',()=>{
  let value=theme.value
if(value==="2")
{
 document.querySelector("#mainin").style.backgroundColor="#090909"
 document.querySelector("#quesdiv").style.backgroundColor="#1E1C1D"
 document.querySelector("#quesdiv").style.border="2px solid #9A9498"
 document.body.style.color="white";

}
else if(value=="1")
{
  document.querySelector("#mainin").style.backgroundColor="#6E4424"
  document.querySelector("#quesdiv").style.backgroundColor="#ffe4c4"
...
// Next question
function nextQ() {
    saveAns(); // Save current choice first
    if (pos < random.length - 1) {
        pos++;
        showQuestions();
    } else {
        // Quiz finished
